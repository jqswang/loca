...
static stamp_tt N_read = 0, N_write = 0; //the number of reads and writes respectively.
...
VOID RecordMem(VOID * ip, VOID * addr, BOOL isRead)
{
  if (isRead){
    N_read++;
  }
  ...
  if(profile_data){
    if (INS_IsMemoryRead(ins)) {
      INS_InsertPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)RecordMem,
			   IARG_INST_PTR, IARG_MEMORYREAD_EA, IARG_BOOL, true, 
			   IARG_END);
    }
    if (INS_HasMemoryRead2(ins)) {
      INS_InsertPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)RecordMem,
			   IARG_INST_PTR, IARG_MEMORYREAD2_EA, IARG_BOOL, true,
			   IARG_END);
    }
    if (INS_IsMemoryWrite(ins))	{
      INS_InsertPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)RecordMem,
			   IARG_INST_PTR, IARG_MEMORYWRITE_EA, IARG_BOOL, false,
			   IARG_END);
    }
  }
...
VOID Fini(INT32 code, VOID *v){
  ...
  ResultFile << "N:" << N << " M1:" << M[0] << " M2:" << M[1] << " total_time:" << time;
  ResultFile << " N_read:" << N_read << " N_write:" << N_write << endl;
